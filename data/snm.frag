...
{
       if (sigtype==0.0)
    return func_hard      (x, a    )*(1.0-func_hard      (x, b    ));
  else if (sigtype==1.0)
    return func_linear    (x, a, sn)*(1.0-func_linear    (x, b, sn));
  else if (sigtype==2.0)
    return func_hermite   (x, a, sn)*(1.0-func_hermite   (x, b, sn));
  else if (sigtype==3.0)
    return func_sin       (x, a, sn)*(1.0-func_sin       (x, b, sn));
  else if (sigtype==4.0)
    return func_smooth    (x, a, sn)*(1.0-func_smooth    (x, b, sn));
  else if (sigtype==5.0)
    return func_atan      (x, a, sn)*(1.0-func_atan      (x, b, sn));
  else if (sigtype==6.0)
    return func_atancos   (x, a, sn)*(1.0-func_atancos   (x, b, sn));
  else if (sigtype==7.0)
    return func_overshoot (x, a, sn)*(1.0-func_overshoot (x, b, sn));
  else if (sigtype==8.0)
    return 1.0/(1.0+exp(-(x-a)*4.0/sn)) * 1.0/(1.0+exp((x-b)*4.0/sn)) * (1.0-0.2*exp(-((x-(a+b)/2.0)*20.0)*((x-(a+b)/2.0)*20.0)));
  else if (sigtype==9.0)
    return 1.0/(1.0+exp(-(x-a)*4.0/sn)) * 1.0/(1.0+exp((x-b)*4.0/sn)) * (1.0+0.2*exp(-((x-(a+b)/2.0)*20.0)*((x-(a+b)/2.0)*20.0)));
}
...
{
  if (mixtype==0.0)
    return x*(1.0-func_hard      (m, 0.5    )) + y*func_hard      (m, 0.5    );
  else if (mixtype==1.0)
    return x*(1.0-func_linear    (m, 0.5, sm)) + y*func_linear    (m, 0.5, sm);
  else if (mixtype==2.0)
    return x*(1.0-func_hermite   (m, 0.5, sm)) + y*func_hermite   (m, 0.5, sm);
  else if (mixtype==3.0)
    return x*(1.0-func_sin       (m, 0.5, sm)) + y*func_sin       (m, 0.5, sm);
  else if (mixtype==4.0)
    return x*(1.0-func_smooth    (m, 0.5, sm)) + y*func_smooth    (m, 0.5, sm);
  else if (mixtype==5.0)
    return x*(1.0-func_atan      (m, 0.5, sm)) + y*func_atan      (m, 0.5, sm);
  else if (mixtype==6.0)
    return x*(1.0-func_atancos   (m, 0.5, sm)) + y*func_atancos   (m, 0.5, sm);
  else if (mixtype==7.0)
    return x*(1.0-func_overshoot (m, 0.5, sm)) + y*func_overshoot (m, 0.5, sm);
}
