...
ComplexToReal::ComplexToReal(const pp::Size& size)
    : size_(size) {
  int w = size_.width();
  int h = size_.height();
  shader_.Init(shader_source_complex_real_frag, shader_source_2tex_vert);
  vb_.SetSize(w, h);
...
void ComplexToReal::Apply(const Texture& in, Texture& out) {
  int w = size_.width();
  int h = size_.height();
  shader_.Use();
