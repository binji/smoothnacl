...
CopyBufferRC::CopyBufferRC(const pp::Size& size)
    : size_(size) {
  int w = size_.width();
  int h = size_.height();
  shader_.Init(shader_source_copybufferrc_frag,
               shader_source_copybufferrc_vert);
  vb_.SetSize(w, h);
...
CopyBufferRC::~CopyBufferRC() {
}

void CopyBufferRC::Apply(const Texture& in, Texture& out) {
  int w = size_.width();
  int h = size_.height();
  shader_.Use();
